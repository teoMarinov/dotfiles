...
hl.bind(mod .. " + SHIFT + W", hl.dsp.exec_cmd(scriptsDir .. "/WallpaperManager.sh"))
...
hl.bind(mod .. " + W", hl.dsp.exec_cmd(scriptsDir .. "/ToggleWaybarAutoHide.sh")) -- toggle waybar auto-hide
...
for _, d in ipairs(deltas) do
	hl.bind(
		mod .. " + SHIFT + " .. d[1],
		hl.dsp.window.resize({ x = d[2] * 50, y = d[3] * 50, relative = true }),
		{ repeating = true }
	)
end
...
for _, d in ipairs(deltas) do
	hl.bind(
		mod .. " + CTRL + SHIFT + " .. d[1],
		hl.dsp.window.move({ x = d[2] * 40, y = d[3] * 40, relative = true }),
		{ repeating = true }
	)
end
